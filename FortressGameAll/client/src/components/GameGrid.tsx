...
export default function GameGrid({ tiles, dwarves, monsters, onTileClick }: GameGridProps) {
  ...
  useEffect(() => {
    ...
    for (const monster of monsters) {
      ...
      ctx.fillStyle = color;
      ctx.fillText(char, monster.x * cellWidth + cellWidth / 2, monster.y * cellHeight + cellHeight / 2);
    }
    
    ctx.fillStyle = 'hsl(350 70% 65%)';
    for (const dwarf of dwarves) {
      ctx.fillText('☺', dwarf.x * cellWidth + cellWidth / 2, dwarf.y * cellHeight + cellHeight / 2);
    }
